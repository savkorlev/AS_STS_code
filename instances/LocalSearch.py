...
def find_best_improvement_2Opt(solution: Solution,
                               instance: Instance) -> bool:  # todo: should probably either optimize for cost or time
    """
    ...
    for r_index, route in enumerate(solution):  # List[Route]
        ...
        if best_2opt is not None:
            solution[r_index] = route[:best_2opt[0]] + list(reversed(route[best_2opt[0]:(best_2opt[1] + 1)])) + route[(
                                                                                                                                  best_2opt[
                                                                                                                                      1] + 1):]
            return True
    return False

def find_best_duration_improvement_2Opt(solution: Solution,
                               instance: Instance) -> bool:  # todo: should probably either optimize for cost or time
    """
    search for the best improving 2-opt move.
    :param solution: list of routes to improve
...
def combine_routes(list_of_routes: list[RouteObject], list_of_avb_vehicles: list[Vehicle], instance: Instance,
                   iteration: int) -> list[RouteObject]:
    for position_a, route_a in enumerate(list_of_routes):
        if route_a.customer_list != [0,
                                     0] and route_a.vehicle.type != "MercedesBenzAtego":  # dont try to combine with Atego-Routes
            ...
            for position_b, route_b in enumerate(list_of_routes):
                if route_b != route_a and route_b.vehicle.type != "MercedesBenzAtego":  # dont try to combine with Atego-Routes
                    if list_of_routes[position_a].customer_list != [0, 0] and list_of_routes[
                        position_b].customer_list != [0, 0]:  # we dont try to combine empty lists

                        temp_list_of_avb_vehicles = list_of_avb_vehicles.copy()
                        temp_list_of_avb_vehicles.append(
                            route_a.vehicle)  # add the vehicles of the two routes into the pool temporarely
                        ...
                        temp_customer_list_ab = [0] + customer_list_a + customer_list_b + [
                            0]  # combining the two lists into one
                        # temp_customer_list_ba = [0] + customer_list_b + customer_list_a + [0]
                        cost_temp_ab, vehicle_temp_ab = find_cheapest_vehicle(temp_customer_list_ab, instance,
                                                                              iteration,
                                                                              temp_list_of_avb_vehicles)  # finding the cheapest vehicle for the new route
                        ...
                        if cost_temp_ab < cost_2routes:  # we operate with first improvement. As soon as we find a combined route cheaper than the two small routes we combine
                            ...
                            list_of_routes[position_a].customer_list = [0,
                                                                        0]  # we set the old routes to empty. This way they will not be chosen in the next iterations of the for loop
